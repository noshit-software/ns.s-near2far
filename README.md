...
- **Map layers** — toggleable overlays on the family map (🔥 wildfire perimeters, 🧊 ICE checkpoints). Fire perimeters are fetched from the USFS ArcGIS public endpoint (`/api/layers/wildfire`, 10-minute server-side cache) and rendered as orange polygons. ICE checkpoint layer is a placeholder pending real crowdsourced source integration. Each layer is a button in the top-right corner of the map; active state is accented orange to match the rest of the glass UI.
