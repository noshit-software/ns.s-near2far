...
- **Map layers** — toggleable overlays on the family map (🔥 wildfire perimeters, 🧊 ICE activity). Fire perimeters are fetched from the Interior/NIFC ArcGIS public endpoint (`/api/layers/wildfire`, 10-minute cache) and rendered as orange polygons with a 🔥 centroid marker; tap for name and acreage. ICE activity is fetched from the StopICE Alerts Network API (`/api/layers/ice?lat=&lng=&distance=75`, 5-minute cache) keyed per-member so each person's area is covered; requires `STOPICE_API_KEY` in the backend env. Tap any 🧊 marker for address, age, confidence (●●●○○), description, and a link to the full stopice.net report. Marker opacity scales with StopICE priority (1=faint → 5=solid) to communicate confidence without using warm colors that could be confused with wildfire markers. Both layer buttons show ⏳ while data is loading. When a member is selected and driving, their direction and speed are projected 10 minutes ahead — if any ICE alert falls within 2 miles of that projected point, a warning chip appears in their detail panel.
